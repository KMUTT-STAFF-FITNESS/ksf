...
import { Table, TableBody, TableHead, TableCell, TableRow, TablePagination, CircularProgress } from '@material-ui/core'
  ...
  return (
    <>
      <Table size="small" {...getTableProps()}>
        <TableHead>
          {headerGroups.map((headerGroup) => (
            <TableRow className="font-kanit" {...headerGroup.getHeaderGroupProps()}>
              {headerGroup.headers.map((column) => (
                <th className="bg-gray-200 text-gray-600 font-normal text-left p-4 text-sm" {...column.getHeaderProps()}>
        ...
        <TableBody {...getTableBodyProps()}>
          {page.map(
            (row) =>
              prepareRow(row) || (
                <TableRow className="hover:bg-gray-100" {...row.getRowProps()} onClick={console.log(row.getRowProps())}>
                  {row.cells.map((cell) => {
                    return (
                      <TableCell {...cell.getCellProps()} className={`${cell.column.className ? cell.column.className : ''}`}>
                        {cell.render('Cell')}
                      </TableCell>
                    )
                  })}
      ...
      <TablePagination
        ...
        labelDisplayedRows={({ from, to, count }) => `รายการที่ ${from}-${to} จากทั้งหมด ${count}`}
      />
    </>
  )
}
